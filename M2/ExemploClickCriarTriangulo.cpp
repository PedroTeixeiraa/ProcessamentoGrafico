...
vector<Triangle> drawnTriangles;

vector <vec3> colors;
int iColor = 0;

vector<vec2> clickedPoints;
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (Triangle &tri : drawnTriangles)
		{
			glBindVertexArray(tri.vao);

			mat4 model = mat4(1.0f); // sem transformações, já desenhado com as coordenadas corretas
...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS)
    {
		...
		clickedPoints.push_back(vec2(xpos, ypos));
