...
int playerX = TILEMAP_WIDTH / 2;
int playerY = TILEMAP_HEIGHT / 2;
...
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
    ...
    {
        if (key == GLFW_KEY_W && playerY > 0) playerY--; // Norte
        if (key == GLFW_KEY_S && playerY < TILEMAP_HEIGHT - 1) playerY++; // Sul
        if (key == GLFW_KEY_D && playerX < TILEMAP_WIDTH - 1) playerX++; // Leste
        if (key == GLFW_KEY_A && playerX > 0) playerX--; // Oeste

        if (key == GLFW_KEY_Q && playerX > 0 && playerY > 0) { playerX--; playerY--; } // Noroeste
        if (key == GLFW_KEY_E && playerX < TILEMAP_WIDTH - 1 && playerY > 0) { playerX++; playerY--; } // Nordeste
        if (key == GLFW_KEY_Z && playerX > 0 && playerY < TILEMAP_HEIGHT - 1) { playerX--; playerY++; } // Sudoeste
        if (key == GLFW_KEY_C && playerX < TILEMAP_WIDTH - 1 && playerY < TILEMAP_HEIGHT - 1) { playerX++; playerY++; } // Sudeste
    }
}
...
void desenharMapa(GLuint shaderID)
{
	...
	for(int i = 0; i<TILEMAP_HEIGHT; i++)
	{
		for (int j=0; j < TILEMAP_WIDTH; j++)
		{
			...
			{
				curr_tile = tileset[0];  // Tile 0 representa o jogador
			}
			else
			{
				curr_tile = tileset[map[i][j]];
			}
